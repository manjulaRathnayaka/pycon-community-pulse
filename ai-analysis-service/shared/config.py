...
class Config:
    ...
    # Try Choreo-injected variables first, then fall back to standard env vars
    _db_host = os.getenv("CHOREO_PYCON_AI_ANALYSIS_DEFAULTDB_HOSTNAME") or os.getenv("DB_HOST")
    _db_port = os.getenv("CHOREO_PYCON_AI_ANALYSIS_DEFAULTDB_PORT") or os.getenv("DB_PORT")
    _db_user = os.getenv("CHOREO_PYCON_AI_ANALYSIS_DEFAULTDB_USERNAME") or os.getenv("DB_USER")
    _db_password = os.getenv("CHOREO_PYCON_AI_ANALYSIS_DEFAULTDB_PASSWORD") or os.getenv("DB_PASSWORD")
    _db_name = os.getenv("CHOREO_PYCON_AI_ANALYSIS_DEFAULTDB_DATABASENAME") or os.getenv("DB_NAME")

    if all([_db_host, _db_port, _db_user, _db_password, _db_name]):
        # For Choreo-managed databases, don't verify SSL certificates
